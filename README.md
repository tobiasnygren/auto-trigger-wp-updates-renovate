...
2. Wait for the next run, which lands inside the schedule window. Job logs are at [developer.mend.io](https://developer.mend.io/).
...
To test outside the schedule window, add the variable `RENOVATE_FORCE` = `{"schedule":null}` for that run.
...
| No Dependency Dashboard on Bitbucket | Bitbucket Cloud removed Issues in August 2026, so the pipeline disables the dashboard. |
